...
export default class SectionsContainer extends React.Component {
  ...
  _handleSectionTransition(index) {
    const position = 0 - (index * this.state.windowHeight) - 66;
    ...
    touchsurface.addEventListener('touchend', (e) => {
      ...
      if (elapsedTime <= allowedTime) { // first condition for awipe met
        if (Math.abs(distY) >= threshold && Math.abs(distX) <= restraint) { // 2nd condition for vertical swipe met
          ...
          let direction = swipedir === 'down' ? that.state.activeSection - 1 : swipedir === 'up' ? that.state.activeSection + 1 : -1;
  ...
  renderNavigation() {
    ...
    const anchors = this.props.anchors.map((link, index) => {
      ...
      return (
        <a
href={`#${link}`} key={index} className={this.props.navigationAnchorClass || 'Navigation-Anchor'}
